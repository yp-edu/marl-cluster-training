...
class ExtraMlp(Model):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.num_extra_dims = kwargs.pop("num_extra_dims", 0)
        super().__init__(
        ...
        self.input_features = sum(
            [
                torch.prod(torch.tensor(spec.shape[-(self.num_extra_dims + 1) :])).item()
    ...
    def _perform_checks(self):
        ...
        for input_key, input_spec in self.input_spec.items(True, True):
            if (self.input_has_agent_dim and len(input_spec.shape) == 2 + self.num_extra_dims) or (
                not self.input_has_agent_dim and len(input_spec.shape) == 1 + self.num_extra_dims
            ):
                if input_shape is None:
                    input_shape = input_spec.shape[: -(self.num_extra_dims + 1)]
                else:
                    if input_spec.shape[: -(self.num_extra_dims + 1)] != input_shape:
                        raise ValueError(
                            f"MLP inputs should all have the same shape up to the last {self.num_extra_dims + 1} dimensions, got {self.input_spec}"
                        )
            else:
                raise ValueError(
    ...
    def _forward(self, tensordict: TensorDictBase) -> TensorDictBase:
        ...
        if self.input_has_agent_dim:
            res = self.mlp.forward(input.to(torch.float32))  # KAZ outputs float64
        ...
        # Does not have multi-agent input dimension
            ...
            if not self.share_params:
                ...
                res = self.mlp[0](input.to(torch.float32))  # KAZ outputs float64
...
@dataclass
class ExtraMlpConfig(ModelConfig):
    """Dataclass config for a :class:`~benchmarl.models.ExtraMlp`."""
